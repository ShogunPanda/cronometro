...
function log(message) {
    var _a;
    const debug = ((_a = process.env.NODE_DEBUG) !== null && _a !== void 0 ? _a : '').includes('cronometro');
    if (debug) {
        currentLogger(`[cronometro ${process.pid.toString().padEnd(5, ' ')} ${new Date(Date.now()).toISOString()}] ${message}`);
    }
}
exports.log = log;
